...
import { handleProp } from '@/utils/utils.ts';
...
const placeholder = computed(() => {
	...
	if (
		...
		return {
			...
			startPlaceholder: search?.props?.startPlaceholder ?? 'start date',
			endPlaceholder: search?.props?.endPlaceholder ?? 'end date',
		};
	}
	const placeholder =
		search?.props?.placeholder ??
		(search?.el?.includes('input') ? 'please enter' : 'please select');
	return { placeholder };
});
